...
const CAREER_DATABASE = [
  {
    ...
    stability: 4,
    trajectoryLinks: [
      { label: ' detailed Roadmap', url: 'https://roadmap.sh/frontend' },
      { label: 'Salary Insights (India)', url: 'https://www.ambitionbox.com/salaries/frontend-developer-salary' }
    ],
    resources: {
      free: [
  ...
  },
  {
    ...
    trajectoryLinks: [
      ...
      { label: 'System Design Interview', url: 'https://github.com/donnemartin/system-design-primer' }
    ],
    resources: {
      free: [
  ...
  },
  {
    ...
    stability: 5,
    trajectoryLinks: [
       { label: 'AI Engineer Roadmap', url: 'https://roadmap.sh/ai-data-scientist' },
  ...
  },
  {
    ...
    stability: 4,
    trajectoryLinks: [
       { label: 'PM Career Path', url: 'https://www.productplan.com/learn/product-manager-career-path/' },
  ...
  },
  {
    ...
    stability: 3,
    trajectoryLinks: [
      { label: 'Growth Marketing Guide', url: 'https://www.drift.com/blog/what-is-growth-marketing/' },
...
const GENERAL_PATH = {
  ...
  stability: 3,
  trajectoryLinks: [
    { label: 'Digital Transformation Guide', url: 'https://enterprisersproject.com/what-is-digital-transformation' }
  ],
  resources: {
...
const generateStrategy = async (profile) => {
  // Simulate "Thinking" time based on complexity
  const responseTime = 500; // Faster backend
  ...
  const generateRoleDetails = (role) => {
    ...
    // Daily Plan - Generate 4 weeks (28 days)
    const dailyPlan = [];
    for (let i = 1; i <= 28; i++) {
      const weekNum = Math.ceil(i / 7);
      const dayInWeek = i % 7 === 0 ? 7 : i % 7;
      let title = "";
      let task = "";

      if (weekNum === 1) {
        if (dayInWeek === 1) { title = 'Gap Analysis'; task = `Deep dive into the "${role.title}" role requirements.`; }
        else if (dayInWeek === 2) { title = 'Environment Setup'; task = `Configure ${role.tools[0]} and initialize a local repo.`; }
        else if (dayInWeek === 3) { title = 'Theory Kickoff'; task = `Watch a crash course on ${role.requiredSkills[0]}.`; }
        else if (dayInWeek === 4) { title = 'Hello World'; task = `Build a basic implementation using ${role.tools[0]}.`; }
        else if (dayInWeek === 5) { title = 'Networking'; task = `Follow 3 industry leaders in ${role.title} on LinkedIn.`; }
        else if (dayInWeek === 6) { title = 'Project Draft'; task = `Sketch a concept for a project using ${role.requiredSkills[1]}.`; }
        else { title = 'Weekly Review'; task = `Review Week 1 progress and set goals for Week 2.`; }
      } else if (weekNum === 2) {
        if (dayInWeek === 1) { title = 'Deep Core'; task = `Study advanced concepts of ${role.requiredSkills[0]}.`; }
        else if (dayInWeek === 2) { title = 'Tool Mastery'; task = `Explore features of ${role.tools[1] || 'advanced debugging tools'}.`; }
        else if (dayInWeek === 3) { title = 'Implementation'; task = `Add feature-rich updates to your Week 1 project.`; }
        else if (dayInWeek === 4) { title = 'Code Review'; task = `Find an open source project in ${role.title} and read 5 PRs.`; }
        else if (dayInWeek === 5) { title = 'Market Pulse'; task = `Research top 5 Indian companies hiring for ${role.title}.`; }
        else if (dayInWeek === 6) { title = 'Skill Extension'; task = `Begin learning ${role.requiredSkills[2] || 'unit testing'}.`; }
        else { title = 'Weekly Review'; task = `Audit your repos and ensure code quality.`; }
      } else {
        // weeks 3 & 4 - dynamic placeholders
        title = `Advanced ${dayInWeek}`;
        task = `Implement ${role.requiredSkills[dayInWeek % role.requiredSkills.length]} in a production-like scenario.`;
        if (dayInWeek === 7) { title = 'Module Review'; task = `Finalize the current learning block.`; }
      }

      dailyPlan.push({ day: `Day ${i}`, week: weekNum, title, task });
    }
